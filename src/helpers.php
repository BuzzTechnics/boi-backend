...
if (! function_exists('logAction')) {
    /**
     * Queue an audit entry against $model (or the acting user when $model is null).
     * Resolves the actor from the request user, falling back to a bound `webhook_user_id`.
     */
    function logAction($model, $action, $changes = null, $user_id = null, $ip_address = null): void
    {
        $user_id ??= request()->user()?->id;
        $user_id ??= app()->bound('webhook_user_id') ? app('webhook_user_id') : null;

        \Boi\Backend\Jobs\LogAction::dispatch($model, $action, $changes, $user_id, ip_address: $ip_address);
    }
}

if (! function_exists('json_to_keyvalue_fields')) {
    /**
     * Build read-only Nova KeyValue/Text detail fields from a JSON array/object.
     * Requires Nova in the host app (provided by the consuming intervention app).
     */
    function json_to_keyvalue_fields($jsonData, $fieldName, $config = [])
    {
        ...
        if (! is_array($items) || empty($items)) {
            return [\Laravel\Nova\Fields\Text::make($fieldName)
        ...
        foreach ($items as $index => $item) {
            ...
            $fields[] = \Laravel\Nova\Fields\KeyValue::make($label)
...
if (! function_exists('apply_routing')) {
    /**
     * Scope a transaction query to the authenticated user's remit: project officer
     * (role 3) → own; regional (role 4) → region states; state (role 6) → own state.
     */
    function apply_routing($query)
    {
        $user = auth()->user();
        ...
        return $query
            ->when($user->role_id == 3, function ($q) use ($user) {
                return $q->where('project_officer_id', $user->id);
            })
            ->when($user->role_id == 4, function ($q) use ($user) {
                $ids = \Boi\Backend\Models\State::where('region_id', $user->region_id)->pluck('id')->toArray();

                return $q->whereIn('state_id', $ids);
            })
            ->when($user->role_id == 6 && $user->state_id !== null, function ($q) use ($user) {
                return $q->where('state_id', $user->state_id);
            });
    }
}

if (! function_exists('apply_officer_routing')) {
    /**
     * Scope an officer/region query to the authenticated user's remit:
     * regional (role 4) → own region; state (role 6) → own state.
     */
    function apply_officer_routing($query)
    {
        $user = auth()->user();
